...
def load_data(dataset_name):
    ...
    if dataset_name in ['seed3', 'seed4']:
        ...
        return data, label
    elif dataset_name in ['deafseed3']:
        allmats = os.listdir(dataset_path[dataset_name])
        data = [[0] * 15]
        label = [[0] * 15]
        for i in range(len(allmats)):
            mat_path = dataset_path['deafseed3'] + '/' + allmats[i]
            one_sub_data = scio.loadmat(mat_path)['data']
            one_sub_label = scio.loadmat(mat_path)['label']
            one_sub_data = np.reshape(one_sub_data, [-1, 310])

            one_sub_label = np.array(one_sub_label)
            one_sub_data = one_sub_data.astype(np.float32)
            one_sub_label = one_sub_label.astype(np.int64)
            # one_sub_data = norminy(one_sub_data)
            min_max_scaler = preprocessing.MinMaxScaler(feature_range=(-1, 1))
...
def create_logger(args):
    ...
    log_file = 'CrossSubject_C3DA_'+ args.dataset + '_lr_' + str(args.lr) + '_seed_' + str(args.seed) + '_{}.log'.format(time_str)
